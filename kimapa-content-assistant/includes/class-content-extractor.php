...
class Content_Extractor
{
    public function extract(int $post_id): array
    {
        ...
        if (trim($raw) === '') {
            $status = 'empty';
            $warnings[] = __('Der rohe Beitragstext ist leer.', 'kimapa-content-assistant');
        }

        if ($content === '') {
            $status = 'empty';
            $warnings[] = __('Der Beitragstext konnte nicht extrahiert werden. Bitte Avada/Fusion-Builder-Struktur prüfen.', 'kimapa-content-assistant');
        } elseif ($word_count < 80) {
            $status = 'short';
            $warnings[] = __('Der extrahierte Beitragstext ist sehr kurz. Bitte prüfen, ob Builder-Inhalte vollständig erkannt wurden.', 'kimapa-content-assistant');
        }
